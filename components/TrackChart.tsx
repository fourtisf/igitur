 ...
 */
export function trackSeries(b: Book) {
  const bs = series(b.seed, N, bookDrift(b.seed), 1.05);
  const ss = series(fnv("SPY:20260907"), N, SPY_DRIFT, 0.55);
  return { bs, ss, bEnd: bs[N - 1], sEnd: ss[N - 1], win: bs[N - 1] >= ss[N - 1] };
}

export function TrackChart({ book }: { book: Book }) {
  const { bs, ss, bEnd, sEnd } = trackSeries(book);
