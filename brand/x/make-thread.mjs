...
/* Card 5: one line of the record. */
.rec{display:flex;flex-direction:column;gap:20px;padding:34px 38px;border-radius:16px;
  background:rgba(255,255,255,.026);box-shadow:inset 0 0 0 1px var(--bd-2)}
.rec .date{font-size:19px;color:var(--ac-2);letter-spacing:.06em;text-transform:uppercase}
.rec .said{font-size:38px;line-height:1.25;letter-spacing:-.025em;font-weight:500}
.rec .note{font-size:21px;color:var(--fg-3);font-weight:350}
`;
...
const CARDS = {
  ...
  <div>
    ...
    <h1 class="mid wide" style="margin-top:34px">No theme in this universe carries that claim.</h1>
  </div>
  <div style="display:flex;flex-direction:column;gap:16px">
    <p class="kick">You wrote</p>
    <p class="typed">“<span>${book.refused}</span>”</p>
  </div>
  <div class="foot">
    <span class="why">It stops, rather than assembling something plausible out of whatever was nearest.</span>
    <span class="meta">${book.themeCount} themes · ${book.nameCount} names</span>
  </div>`),

  "04-benchmark": shell("4 / 5", `
  <div>
    <h1>Roughly half of all books <em>lose</em> to the index.</h1>
    <p class="sub">That is deliberate. An earlier build gave every book a higher drift than the
      benchmark, so everything beat the market — a claim the interface was making on its own.</p>
  </div>
  <div class="foot">
    <span class="why">Nothing here should be able to flatter itself.</span>
    <span class="meta">The benchmark is drawn either way</span>
  </div>`),

  "05-record": shell("5 / 5", `
  <div>
    <h1 class="mid">Put a claim on the record.</h1>
    <p class="sub">The date is written by the server the day you commit it. Not in the link, not
      editable afterwards — which is the only reason the figure beside it later means anything.</p>
  </div>
  <div class="rec">
    <span class="date">On the record · stated 9 September 2026</span>
    <span class="said">“${book.premise}”</span>
    <span class="note">Measured against the index from that date. Win or lose, it stays up.</span>
  </div>
  <div class="foot">
    <span class="why">Nothing is removed for having been wrong.</span>
